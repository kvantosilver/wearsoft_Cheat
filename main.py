...
class MainWearDesign(QMainWindow):
    ...
    def import_xlxs(self):
        ...
        if ok_pressed:
            quarter, ok_pressed = QInputDialog.getItem(self, "Квартал",
            ...
            win.exec()
            return ''
        collection350 = pd.read_excel(fname, sheet_name='до 350').to_csv(index=False).split('\r\n')[2:]
        collection650 = pd.read_excel(fname, sheet_name='351-650').to_csv(index=False).split('\r\n')[2:]
        collection1200 = pd.read_excel(fname, sheet_name='651-1200').to_csv(index=False).split('\r\n')[2:]
        self.file = open("data/setting.txt", encoding="UTF-8")
        data = self.file.readlines()
        self.connection = mysql.connector.connect(
            host=data[0][:-1],
        ...
        for element in collection350:
            ...
            for index in range(len(list_element)):
                if index == 9:
                    ...
                    if list_element[11] != '':
                        pin.append(str(round(float(list_element[11]) / float(stress_data[0]), 5)))
                        pin.append(list_element[index])
                    else:
                        pin.append(str(round(float(list_element[12]) / float(stress_data[0]), 5)))
                        pin.append(list_element[index])
                else:
                    pin.append(list_element[index])
